(function($){
    ...
    $(window).load(function(){
        ...
        if (checkIe8.length > 0 || checkIe7.length > 0 || checkIe6.length > 0) {
            window.location.replace("/Prepare/Index");
        }
    ...
    function init_classic_menu_resize(){
        ...
        if ($(window).width() < 1024) {
            ...
            //$(".main-nav").addClass("transperency-nav");
        }
        else 
            if ($(window).width() >= 1024) {
    ...
    function init_classic_menu(){
        ...
        mnThisLi.hover(function () {
            ...
            if (!($(".main-nav").hasClass("mobile-on"))) {
                ...
                /* dj */
                var items = $('.hetekee-nav-item-on');
                _.each(items, function (item) {
                    $(item).stop(true, true).delay(100).fadeOut("fast");
                });

                $(this).find("img.hetekee-nav-item-on:first").stop(true, true).fadeIn("fast");

                var menuTitle = $(this).find('.mn-has-sub');
                var margin = $(menuTitle).position().left;
                if ($(menuTitle).hasClass('aboutus')) {
                    var menuWidth = $(menuTitle).width();
                    var subWidth = $($($(menuTitle).parent()).find('.mn-sub')).children('li').width();
            ...
            if (!($(".main-nav").hasClass("mobile-on"))) {
                ...
                /* dj */
                var items = $('.hetekee-nav-item');
                _.each(items, function (item) {
                    if ($(item).hasClass('selected')) {
                        $(item).find(".hetekee-nav-item-on:first").stop(true, true).fadeIn("fast");
                    } else {
                        $(item).find(".hetekee-nav-item-on:first").stop(true, true).delay(100).fadeOut("fast");
                    }
                });
            }
